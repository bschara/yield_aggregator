...
public entry fun execute_intent(
    ...
    match intent.action {
        0 => deposit_to_strategy(vault, registry, &intent),
        1 => withdraw_from_strategy(vault, registry, &intent),
        2 => harvest_strategy(vault, registry, &intent),
        3 => emergency_exit_strategy(vault, registry, &intent),
        _ => assert!(false, 20)
    };
